...
public class CartController {
    ...
    public ApiResult<List<CartDto>> all() {
        var dtos = cartService.all(ApiContextHolder.getShopId(), ApiContextHolder.getAuthDto().getUserId());
    ...
    public ApiResult<Integer> countProd() {
        var count = cartService.countProd(ApiContextHolder.getShopId(), ApiContextHolder.getAuthDto().getUserId());
    ...
    public ApiResult<String> add(@RequestBody CartAddParam cartAddParam) {
        var ret = cartService.add(ApiContextHolder.getShopId(), ApiContextHolder.getAuthDto().getUserId(),
                cartAddParam);
    ...
    public ApiResult clear() {
        var ret = cartService.clear(ApiContextHolder.getShopId(), ApiContextHolder.getAuthDto().getUserId());
        return ApiResult.fromTupleRet(ret);
    }
